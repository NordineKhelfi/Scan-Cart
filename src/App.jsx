...
export default function App() {
  ...
  const handleScan = async (barcode) => {
    ...
    if (currentStore) {
      ...
      priceData = await db.prices.where({ storeId: currentStore.id, barcode }).first() 
                  || await db.prices.where('barcode').equals(barcode).and(p => p.storeId === currentStore.id).first(); 
    }
  ...
  const handleSaveItem = async (itemData) => {
    ...
    if (barcode) {
      ...
      if (currentStore) {
        const existingPrice = await db.prices.where('barcode').equals(barcode).and(p => p.storeId === currentStore.id).first();
        if (existingPrice) {
          await db.prices.update(existingPrice.id, { price, updatedAt: Date.now() });
        } else {
          await db.prices.add({
            id: uuidv4(),
  ...
  const handleFinishTrip = async () => {
    if (window.confirm('Are you sure you want to finish this trip and clear your cart?')) {
      await db.cart.clear();
      setCartItems([]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      <div style={{ position: 'absolute', top: '15px', right: '15px', zIndex: 30 }}>
         <button className="btn btn-secondary" onClick={handleManualItem} style={{ borderRadius: '2rem', padding: '0.5rem 1rem', boxShadow: 'var(--shadow-md)' }}>
           + Add Manual
         </button>
      </div>
      
      {currentStore && (
        <div className="glass-badge" style={{ position: 'absolute', top: '15px', left: '15px', zIndex: 30 }}>
          🛍️ {currentStore.name}
        </div>
      )}
